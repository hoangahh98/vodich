...
export class AdminController {
  ...
  async deleteAdmin(@Req() req: Request, @Res() res: Response, @Param('id') id: string) {
    const adminId = parseBigId(id);
    if (!adminId) return notFound(res, 'Không tìm thấy tài khoản');
    await this.adminService.deleteDelegatedAdmin(adminId, req.session.user?.id);
    return res.redirect('/permissions');
  }
  ...
  async logs(@Req() req: Request, @Res() res: Response) {
    ...
    const [logs, users] = await Promise.all([this.adminService.listLogs(level, category, user), this.adminService.listLogUsers(level, category)]);
